...
AudioSink::AudioSink() 
    : 
    format_(Format::WAV),
    filename_("output.wav"),
    sample_rate_(48000),
    sample_count_(0), // TODO: init sample count should always be 0; therefore, shouldn't be a param?
...
AudioSink::~AudioSink() 
{
    std::cout << "AudioSink destructor" << std::endl;

    finalize(); 
}

void AudioSink::process(const AudioData& audio)
{
    //std::cout << "AudioSink received " << audio.size() << " samples" << std::endl;
...
// TODO:
// void AudioSink::process(const std::vector<float> &audio)
// {
...
void AudioSink::write(const std::vector<float>& audio) 
{
    ...
    //std::cout << "sample_count_: " << sample_count_ << std::endl;
...
{
    std::cout << "AudioSink::finalize()" << std::endl;
...
double AudioSink::getDurationSeconds() const
{
    return static_cast<double>(sample_count_) / sample_rate_;
}